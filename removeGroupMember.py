...
def remove_group_member(conn):
    changes = {
        "member": [(MODIFY_DELETE, [MEMBER_DN])],
    }

    if conn.modify(dn=GROUP_DN, changes=changes):
        print(f"[SUCCESS] Member removed from group '{GROUP_NAME}'")
        print(f"  Member: {MEMBER_DN}")
        return True

    print("[ERROR] Failed to remove member from group")
    print(conn.result)
    return False
